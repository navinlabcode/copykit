...
After `runVarbin()` we can use the bincounts data to calculate ratios. It is possible to select the assay that will be used to calculate the ratios. For this tutorial we use the variance stabilized transformed bincounts by setting the argument assay to 'ft'. To use the original bincount matrix the argument assay can be set to 'bin_counts'. `calcRatios()` will store the calculated ratios within the scCNA object, which can be accessed with `ratios()`

```{r}
breast_tumor <- calcRatios(breast_tumor, assay = "ft")
breast_tumor
```
...
Some datasets contain cells with flat profiles that are likely to be diploid cells. CopyKit provides a function to automatically detect those cells. With `findNormalCells()` copykit classifies the samples according to the coefficient of variation from the segment ratios. By default, copykit automatically attempts to find a resolution value that will detect those cells. After running this function it is highly recommended to visually inspect the results using a heatmap (see `plotHeatmap()`) and umap.

```{r}
breast_tumor <- copykit::findNormalCells(breast_tumor)
plotUmap(breast_tumor,
         label = "is_normal")
plotHeatmap(breast_tumor,
            label = "is_normal")
```

The information added to the metadata can now be used to filter these cells out of the dataset.
```{r}
breast_tumor <- breast_tumor[,SummarizedExperiment::colData(breast_tumor)$is_normal == FALSE]
```
...
```{r}
opt_k <- findOptimalK(breast_tumor,
             k_range = 13:30)$k
```
...
copykit::plotRatio(breast_tumor)
```
...
```{r}
copykit::plotGeneCopy(breast_tumor,
                      geom = "swarm",
                      genes = c("KRAS",
                                "MYC",
                                "TP53",
...
This function remains as a legacy and will only receive critical breaking updates.
